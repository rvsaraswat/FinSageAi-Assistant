 ...
 */
async function callKiteAPI(endpoint, options = {}) {
...
export async function getHoldings() {
  ...
  const holdings = await callKiteAPI('/portfolio/holdings');
...
export async function getPositions() {
  ...
  const positions = await callKiteAPI('/portfolio/positions');
  return positions;
}
...
export async function getQuotes(instruments) {
  ...
  const quotes = await callKiteAPI(`/quote?${params.toString()}`);
  return quotes;
}
...
export async function getProfile() {
  ...
  const profile = await callKiteAPI('/user/profile');
  return profile;
}
